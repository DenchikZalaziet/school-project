...
from backend.app.utils.db_utils import (
    get_instrument_tunings_collection,
...
def get_instrument_tuning_notes(
    tuning_id: str,
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Неверный ID настройки",
        )

    if not tuning:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Строй не найден"
        )

    found_tuning = Tuning.model_validate(tuning)

    try:
        instrument = instruments_collection.find_one(
            {"_id": ObjectId(tuning["instrument_id"])}
        )
    except InvalidId:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Неверный ID инструмента",
        )
    ...
    try:
        notes = get_instrument_notes(
            instrument=found_instrument, tuning=found_tuning, prefer_flats=prefer_flats
        )
...
def get_instrument_tuning_scale_notes(
    tuning_id: str,
    scale_id: str,
    root: str,
    ...
    instruments_collection=Depends(get_instruments_collection),
    tunings_collection=Depends(get_instrument_tunings_collection),
    scales_collection=Depends(get_scales_collection),
) -> list[list[str]]:
    try:
        tuning = tunings_collection.find_one({"_id": ObjectId(tuning_id)})
    except InvalidId:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Неверный ID гаммы"
        )
