...
from backend.app.utils.db_utils import get_scales_collection, get_users_collection
...
async def edit_user_me(
    data: Annotated[UserEditForm, Form()],
    current_user: Annotated[User, Depends(get_current_active_user)],
    ...
    if (
        ...
        and collection.count_documents({"username": data.username}) > 0
    ):
        raise HTTPException(
    ...
    result = collection.update_one(
        {"_id": current_user_objectId}, {"$set": data.model_dump(exclude_none=True)}
    )
    if not result.acknowledged:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Не удалось изменить пользователя",
        )
...
async def get_my_scales(
    ...
    my_scales_amount = collection.count_documents(
        {"owner_id": user_id, "name": {"$regex": query}}
    )
    if length == 0:
    ...
    my_scales_cursor = (
        collection.find({"owner_id": user_id, "name": {"$regex": query}})
        .sort("name")
        .skip(length * (page - 1))
        .limit(length)
    )
