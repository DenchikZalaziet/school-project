...
async def create_scale(
    ...
    collection: Collection = Depends(get_scales_collection),
) -> Scale:
    if scale.category == DEFAULT_SCALE_NAME_CHANGE_PREVENT:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"Категория не может быть {DEFAULT_SCALE_NAME_CHANGE_PREVENT}",
        )
...
@scales_router.get("")
async def get_public_scales(
    length: int = 0,
    page: int = 1,
    query: str = "",
    collection: Collection = Depends(get_scales_collection),
) -> dict[str, Union[int, list[Scale]]]:
    if length is not None and length < 0:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Длина должна быть больше нуля или ноль",
        )
    if page is not None and page < 1:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Номер страницы должен быть больше нуля",
        )

    scales_amount = collection.count_documents(
        {"public": True, "name": {"$regex": query}}
    )

    safe_length = length if length > 0 else 10 
    safe_page = page if page > 0 else 1
    pages_count = math.ceil(scales_amount / safe_length)

    public_scales_cursor = (
        collection.find({"public": True, "name": {"$regex": query, "$options": "i"}})
        .sort("name")
        .skip(safe_length * (safe_page - 1))
        .limit(safe_length)
    )
...
async def get_scale_by_id(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Неверный ID"
        )

    if not scale:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Гамма не найдена"
        )
    ...
    if current_user is None:
        raise HTTPException(
            ...
            detail="Приватная гамма не может быть просмотрена без входа в аккаунт",
        )
    ...
    if found_scale.owner_id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Гамма не может быть просмотрена текущим пользователем",
        )
    return found_scale


@scales_router.delete("/{scale_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_scale_by_id(
    scale_id: str,
    current_user: Annotated[User, Depends(get_current_active_user)],
    collection: Collection = Depends(get_scales_collection),
) -> None:
    ...
    if found_scale.owner_id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Гамма не может быть изменена текущим пользователем",
        )

    result = collection.delete_one({"_id": ObjectId(scale_id)})
    if not result.acknowledged:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Не удалось удалить гамму"
        )


@scales_router.patch("/{scale_id}", status_code=status.HTTP_204_NO_CONTENT)
async def edit_scale_by_id(
    scale_id: str,
    data: ScaleEditForm,
    current_user: Annotated[User, Depends(get_current_active_user)],
    collection: Collection = Depends(get_scales_collection),
    ...
    if (
        scale["category"] == DEFAULT_SCALE_NAME_CHANGE_PREVENT
        or data.category == DEFAULT_SCALE_NAME_CHANGE_PREVENT
    ):
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"Категория не может быть {DEFAULT_SCALE_NAME_CHANGE_PREVENT}",
    ...
    result = collection.update_one(
        {"_id": current_scale_objectId}, {"$set": data.model_dump(exclude_none=True)}
    )
    if not result.acknowledged:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Не удалось изменить гамму"
        )


@scales_router.get("/{scale_id}/notes")
async def get_scale_notes_by_id(
    scale_id: str,
    current_user: Annotated[User, Depends(get_optional_active_user)],
    root: str = "C",
    ...
    if not found_scale.public and found_scale.owner_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Приватная гамма не может быть просмотрена текущим пользователем",
        )

    try:
        result = get_scale_notes(
            root=root, scale=found_scale, prefer_flats=prefer_flats
        )
