...
from backend.app.schemas.user_schemas import Token, UserInDB
...
async def register(
    ...
    try:
        user = UserInDB(
            username=form_data.username, disabled=False, hashed_password=hashed_password
        )
    except ValidationError:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Произошла ошибка"
        )
