...
def test_full_scale_flow(client):
    ...
    response = client.post(
        ...
        json={"name": "test_scale_wrong_token", "intervals": [], "category": "invalid"},
    )
    ...
    response = client.post(
        "/scale",
        headers={"Authorization": f"Bearer {token}"},
        json={"name": "test_scale1", "intervals": [], "public": True},
    )
    assert response.status_code == 201
    ...
    response = client.post(
        "/scale",
        headers={"Authorization": f"Bearer {token}"},
        json={"name": "test_scale2", "intervals": [1, 2], "public": True},
    )
    assert response.status_code == 201
    ...
    response = client.post(
        "/scale",
        headers={"Authorization": f"Bearer {token}"},
        json={"name": "wrong_scale", "intervals": [-1], "category": "invalid"},
    )
    ...
    response = client.get(
        "/user/me/scale", headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200
    ...
    response = client.get(
        f"/scale/{scale_id}", headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200
    ...
    response = client.get(
        f"/scale/{scale_id}/notes",
        headers={"Authorization": f"Bearer {token}"},
        params={"root": "A♯", "prefer_flats": True},
    )
    assert response.status_code == 200
    ...
    response = client.delete(
        f"/scale/{fake_scale_id}", headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 404
...
def test_scale_private(client):
    ...
    response = client.post(
        "/auth/register",
        data={"username": "test_user2", "password": "secure_password123"},
    )
    assert response.status_code == 201
    ...
    response = client.get(
        f"/scale/{scale_id}", headers={"Authorization": f"Bearer {token2}"}
    )
    assert response.status_code == 403

    response = client.get(
        f"/scale/{scale_id_public}", headers={"Authorization": f"Bearer {token2}"}
    )
    assert response.status_code == 200
    ...
    response = client.patch(
        ...
        json={"name": "edited_name"},
    )
    assert response.status_code == 403

    response = client.patch(
        f"/scale/{scale_id_public}",
        headers={"Authorization": f"Bearer {token2}"},
        json={"name": "edited_name"},
    )
    assert response.status_code == 403
    ...
    response = client.delete(
        f"/scale/{scale_id}", headers={"Authorization": f"Bearer {token1}"}
    )
    assert response.status_code == 204
